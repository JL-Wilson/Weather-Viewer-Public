...
class _ForecastPageState extends State<ForecastPage> {
  ...
  Widget build(BuildContext context) {
    List<Widget> createDaySelection() {
      int offset = 0;
      ...
      List<Widget> daySelection = List<Widget>.generate(
        _latestData.returnForecasts().length,
        (int index) {
          return Padding(
            padding: const EdgeInsets.only(right: 4),
            child: ChoiceChip(
              iconTheme: _value == index
                  ? IconThemeData(
              ...
              label: Text(_latestData.returnForecasts()[index].shortDate(),
                  style: _value == index
      ...
      return daySelection.sublist(offset);
    }
            ...
            child: Row(
              ...
              children: List<Widget>.generate(
                ...
                (int index) {
                  DateFormat customTimeFormat = DateFormat("HH");
                  int currentTime =
                      int.parse(customTimeFormat.format(DateTime.now()));

                  if ((_latestData.returnForecasts()[index].date ==
                          DateFormat("yyyy-MM-dd").format(DateTime.now())) &&
                      (index == 0) &&
                      (_latestData
